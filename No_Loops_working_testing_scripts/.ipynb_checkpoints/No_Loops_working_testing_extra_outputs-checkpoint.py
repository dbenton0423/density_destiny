...
outpath = '/work/08006/dbenton/stampede3/density_destiny/allgas/Testing/'
...
selection = (gas_gen == 0) & (gas_child == 0) #& (gas_feh > -15)
...
np.savetxt(outpath + 'ids/' + 'ids' + sstart + '.txt', ids[:,0])
...
#np.savetxt(outpath + 'initial_ids_' + sstart, ids[:,0])
#np.savetxt(outpath + 'initial_low_feh' + sstart, ids[:,0])
...
for i in range(len(snapshot_number)):
    ...
    selection = (gas_gen_z == 0) & (gas_child_z == 0)
    gas_ids_selected_z = gas_ids_z[selection]
    gas_den_selected_z = gas_den_z[selection]*((MsunToGm/KpcToCm**3)/mp)        
                   
    
    
    #gas_feh_z = part_z['gas'].prop('metallicity.fe')
