...
outpath = '/work/08006/dbenton/stampede3/'
...
part = gizmo.io.Read.read_snapshots(['star','gas'],'index', start
...
selection = (gas_gen == 0) & (gas_child == 0) #& (R_0 < 30)
gas_ids_selected = gas_ids[selection]


### SET HIGH/LOW FEH ###
high_feh = gas_feh >= np.log(0.6)
low_feh = gas_feh < np.log(0.6)


gas_feh = gas_feh[selection]


#gas_ids_high_feh_selected = gas_ids_selected[high_feh]
#gas_ids_low_feh_selected = gas_ids_selected[low_feh]

gas_ids_selected_sorted = np.sort(gas_ids_selected)

#gas_ids_high_feh_selected_sorted = np.sort(gas_ids_high_feh_selected)
#gas_ids_low_feh_selected_sorted = np.sort(gas_ids_low_feh_selected)


dosort = np.argsort(gas_ids_selected)
#high_feh_dosort = np.argsort(gas_ids_high_feh_selected)
#low_feh_dosort = np.argsort(gas_ids_low_feh_selected)

gas_den_selected = gas_den[selection]
#high_feh_gas_den_selected = gas_den_selected[high_feh]
#low_feh_gas_den_selected = gas_den_selected[low_feh]





print(len(gas_ids_selected))
...
snapshot_number = range(start + 1 , end, 1)
ids = np.zeros([len(gas_ids_selected_sorted), (len(snapshot_number) + 1)], dtype=int)
#high_feh_ids = np.zeros([len(gas_ids_high_feh_selected_sorted), (len(snapshot_number) + 1)], dtype=int)
#low_feh_ids = np.zeros([len(gas_ids_low_feh_selected_sorted), (len(snapshot_number) + 1)], dtype=int)

ids[:,0] = gas_ids_selected_sorted
#high_feh_ids[:,0] = gas_ids_high_feh_selected_sorted
#low_feh_ids[:,0] = gas_ids_low_feh_selected_sorted



gas_den_ncm3_selected = gas_den_selected*((MsunToGm/KpcToCm**3)/mp)
#high_feh_gas_den_ncm3_selected = high_feh_gas_den_selected*((MsunToGm/KpcToCm**3)/mp)
#low_feh_gas_den_ncm3_selected = low_feh_gas_den_selected*((MsunToGm/KpcToCm**3)/mp)

den = np.zeros([len(gas_ids_selected_sorted), (len(snapshot_number) + 1)])
#high_feh_den = np.zeros([len(gas_ids_high_feh_selected_sorted), (len(snapshot_number) + 1)])
#low_feh_den = np.zeros([len(gas_ids_low_feh_selected_sorted), (len(snapshot_number) + 1)])

den[:,0] = gas_den_ncm3_selected[dosort]
gas_feh = gas_feh[dosort]

#high_feh_den[:,0] = high_feh_gas_den_ncm3_selected[high_feh_dosort]
#low_feh_den[:,0] = low_feh_gas_den_ncm3_selected[low_feh_dosort]
...
#high_feh_destined = np.ones(len(den[:,0]), dtype = int)
...
for i in range(len(snapshot_number)):
    ...
    selection_z = (gas_gen_z == 0) & (gas_child_z == 0) #& (R_z < 30)
    ...
    for j in range(len(destined)):
        if destined[j] == 1:
            if den[j,i+1] == 0:
                ...
                if gas_feh[j] >= np.log(0.6):
                    high_feh_inst_is_dense.append(den[j,0])
                else:
                    low_feh_inst_is_dense.append(den[j,0])
                ...
                if gas_feh[j] >= np.log(0.6):
                    high_feh_inst_is_diffuse.append(den[j,0])
                else:
                    low_feh_inst_is_diffuse.append(den[j,0])
...
high_feh_outfilename = sstart + '_' + send + '_high_feh.txt'
low_feh_outfilename = sstart + '_' + send + '_low_feh.txt'
...
np.savetxt(outpath + 'dense' + high_feh_outfilename, high_feh_inst_is_dense)
np.savetxt(outpath + 'diffuse' + high_feh_outfilename, high_feh_inst_is_diffuse)

np.savetxt(outpath + 'dense' + low_feh_outfilename, low_feh_inst_is_dense)
np.savetxt(outpath + 'diffuse' + low_feh_outfilename, low_feh_inst_is_diffuse)
