...
outpath = '/work/08006/dbenton/stampede3/feh/quarter_z/'
...
destined = np.loadtxt(outpath + 'destined_feh' + str(450) + '_'+ str(475) + '.txt')
...
load_high_dense = np.loadtxt(outpath + 'dense_high_feh' + str(450) + '_'+ str(475) + '.txt')
load_high_diffuse = np.loadtxt(outpath + 'diffuse_high_feh' + str(450) + '_'+ str(475) + '.txt')

load_low_dense = np.loadtxt(outpath + 'dense_low_feh' + str(450) + '_'+ str(475) + '.txt')
load_low_diffuse = np.loadtxt(outpath + 'diffuse_low_feh' + str(450) + '_'+ str(475) + '.txt')
          
load_dense_ids = np.loadtxt(outpath + 'ids/' + 'dense_ids_feh_' + str(450) + '_' + str(475) + '.txt')
load_diffuse_ids = np.loadtxt(outpath + 'ids/' + 'diffuse_ids_feh_' + str(450) + '_' + str(475) + '.txt')
...
for i in range(len(load_high_dense)):
    high_feh_inst_is_dense.append(load_high_dense[i])
                             
for i in range(len(load_low_dense)):
    low_feh_inst_is_dense.append(load_low_dense[i])
        
        
for i in range(len(load_high_diffuse)):
    high_feh_inst_is_diffuse.append(load_high_diffuse[i])        
        
for i in range(len(load_low_diffuse)):
    low_feh_inst_is_diffuse.append(load_low_diffuse[i])
...
for i in range(len(snapshot_number)):
            ...
            if (R_0[j] < 20) & (z[j] > -5) & (z[j] < 5):
                if den[j,i+1] == 0:
                    ...
                    if gas_feh[j] >= median:
                        ...
                        destined[j] = 0
                    elif gas_feh[j] < median and gas_feh[j] >= -15:
    ...
    np.savetxt(outpath + 'ids/' + 'ids_feh_' + str(start + i +1) + '.txt', ids[:,i+1])
...
np.savetxt(outpath + 'destined_feh' + sstart + '_' + send + '.txt', destined)

np.savetxt(outpath + 'dense_high_feh' + outfilename,high_feh_inst_is_dense)
np.savetxt(outpath + 'diffuse_high_feh' + outfilename,high_feh_inst_is_diffuse)

np.savetxt(outpath + 'dense_low_feh' + outfilename,low_feh_inst_is_dense)
np.savetxt(outpath + 'diffuse_low_feh' + outfilename,low_feh_inst_is_diffuse)

np.savetxt(outpath + 'ids/' + 'dense_ids_feh_' + str(start) + '_' + str(end) + '.txt', is_dense)
np.savetxt(outpath + 'ids/' + 'diffuse_ids_feh_' + str(start) + '_' + str(end) + '.txt', is_diffuse)
